...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Each idx is a document with a sequence of characters
        ...
        total_length = len(truncated_document)
        masked_length = random.randint(1, max(1, total_length // 4))
        prefix_length = random.randint(0, total_length - masked_length)
        suffix_length = total_length - prefix_length - masked_length

        prefix = truncated_document[:prefix_length]
        masked_content = truncated_document[
            prefix_length : prefix_length + masked_length
        ]
        suffix = truncated_document[prefix_length + masked_length :]

        # Step 3: Create masked_string
        masked_string = (
            prefix
            + self.MASK_CHAR
            + suffix
            + self.MASK_CHAR
            + masked_content
            + self.PAD_CHAR * (self.block_size + 1 - total_length - 2)
            # -2 for the MASK_CHARs, +1 for the extra character, - length of the document
        )

        # Step 4: Create input and output examples
        x_string = masked_string[:-1]  # -1 to exclude the last character
